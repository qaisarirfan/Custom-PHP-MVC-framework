...
<div class="container">
	<div class="context">
		...
		<form class="form-horizontal" name="edit_background" onSubmit="" method="post" enctype="multipart/form-data">
			<input type="hidden" name="command" value="edit-customize" />
			...
			<div class="control-group">
				<div class="controls">
					<div class="background" style="height:100px; width:200px; background:url('<?php echo $this->get_site_content_path(); ?>/custom-background/<?php echo $this->row['backgrounds_image'] . "') " . $this->row['background_repeat'] . " " . $this->row['background_position'] . " " . $this->row['background_attachment'] . " " . $this->row['backgrounds_color']?>;"></div>
				</div>
			</div>
			...
			<div class="control-group">
				<label class="control-label" for="background_attachment">Background Attachment :</label>
				<div class="controls">
...
<script type="text/javascript">
	<?php 
		$backgrounds_color = str_replace('#','',$this->row['backgrounds_color']);
		$position = explode(' ',$this->row['background_position']); 
	?>
	$(document).ready(function(){
		$('#colorpickerField1').val('<?php echo $backgrounds_color; ?>');
		$('#position_x').val('<?php echo $position[0]; ?>');
		$('#position_y').val('<?php echo $position[1]; ?>');
		$('#background_repeat').val('<?php echo $this->row['background_repeat']; ?>');
		$('#background_attachment').val('<?php echo $this->row['background_attachment']; ?>');
		<?php if($this->row['locked'] == 'yes'){ ?>
